...
class FreeIMUCal(QMainWindow, Ui_FreeIMUCal):
  def __init__(self, parent=None):
    super(FreeIMUCal, self).__init__(parent)
#    QMainWindow.__init__(self)
    ...
    self.serialPortEdit.setText(str(self.settings.value("calgui/serialPortEdit", "")))


    # Connect up the buttons to their functions
    self.serialPortEdit.returnPressed.connect(self.serial_connect)
    ...
    self.magn3D_sp = gl.GLScatterPlotItem(pos=temp_pos)
  ...
  def sampling_start(self):
    ...
    self.serWorker.sig_newdata.connect(self.newData)
