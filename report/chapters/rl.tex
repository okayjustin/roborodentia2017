...
\subsection{Three Actors Implementation}
...
The x translation network takes in the robot's x position and velocity and outputs $u_x$. Since the Y translation network accomplishes the exact same task in the same way but in the y direction, only the X translation network will be discussed. As described above in Actor Testing, the networks are periodically tested to observe their change in performance with more training. Figures \ref{fig:x_r} and \ref{fig:x_q} show the average testing episode reward and average max Q versus episodes trained, respectively. Figures \ref{fig:x_actor_contour} and \ref{fig:x_critic_contour} show contour plots of the actor and critic outputs as functions of the two inputs.
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\caption{X Translation Network Performance -- 1 Episode}\label{fig:x_perf1}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=6in, height=3.85in, keepaspectratio]{figures/train_figs/transx_transitions/1_21.pdf}
	\caption{X Translation Network Performance -- 21 Episode}\label{fig:x_perf21}
\end{figure}
\begin{figure}[H]
	\centering
...
The angle network takes in the robot's angle as $\text{cos}(\theta)$ and $\text{sin}(\theta)$ as well as the rotational velocity $\dot{\theta}$ and outputs $u_\theta$.
\begin{figure}[H]
	\centering
...
\subsection{Single Actor Implementation}
\begin{figure}[H]
